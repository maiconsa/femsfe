...
public class IntersectGeometry {
 	...
 	public static  void compute(){
 			switch (type) {
				case IntersectGeometry.LINES:
					...
					if(linesL.size() <2){ SelectGeometry.clear();return; }
	...
	private static void segmentSegment(Line2D a , Line2D b){
		...
		if(!list.isEmpty()){
			...
			JOptionPane.showMessageDialog(null,"Ponto de interse��o n�o encontrado!!","Ponto de interse��o n�o encontrado", JOptionPane.INFORMATION_MESSAGE);
			a.deselectThis();b.deselectThis();
		}
	}
	...
	private static  List<Line2D> bruteSegmentsIntersections(List<Line2D> segments){		
		...
		for (Line2D line2d : segments) {
			intersections.addAll(line2d.getLinesFromInterPoints());
		}
