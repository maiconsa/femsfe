...
public final class FXGLPanel extends SwingNode implements GLEventListener, MouseInputListener {
    ...
    private void drawArrowsMap(ConectivityList mesh) {
        if (mesh == null || mesh.isArrowsVisible() == false) {
            return;
        }
        for (Triangle2D triangle2d : mesh.getElements()) {
    ...
    private void drawContoursMap(ConectivityList mesh) {
        if (mesh == null || !mesh.isIsolinesVisible()) {
            return;
        }
        if (mesh.getElements().isEmpty()) {
            return;
        }
    ...
    private void drawResultColorMap(ConectivityList mesh) {
        if (mesh == null || mesh.isColorMapVisible() == false) {
            return;
        }
        FiniteAnalysis FEMA = Project.getFiniteAnalysis();
    ...
    private void drawGeometry(Geometry2D geometry2d) {
        ...
        if (geometry2d == null || !geometry2d.isVisible()) {
            return;
        }
    ...
    private void pan() {
        if (getToggleButtonByName("TGLBTN_PAN") == null) {
            return;
        }
