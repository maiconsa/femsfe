...
public class DefineBCSpaceDensityDialog extends TextInputDialog {
	...
	public DefineBCSpaceDensityDialog(ProblemType type) {
		...
		switch (Project.getProblemType()) {
			case ELETROSTATIC:
				this.setGraphic(new ImageView(new Image(getClass().getResourceAsStream("/resources/icon/poisson_equation_eletro.png"))));
			...
			case MAGNETOSTATIC_VECTOR_POTENTIAL:
				this.setGraphic(new ImageView(new Image(getClass().getResourceAsStream("/resources/icon/equation_magne.png"))));
