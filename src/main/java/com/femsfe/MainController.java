...
import com.femsfe.BoundaryCondition.BoundaryConditionType;
import com.femsfe.BoundaryCondition.DirichletConditionExpression;
import com.femsfe.BoundaryCondition.PermanentField;
import com.femsfe.BoundaryCondition.RobinConditionExpression;
import com.femsfe.BoundaryCondition.SpaceDensityFunction;
import com.femsfe.ComplexNumber.ComplexNumber;
import com.femsfe.GUIDialogBox.DefineBCDensityDialog;
import com.femsfe.GUIDialogBox.DefineBCPotentialDialog;
import com.femsfe.GUIDialogBox.DefineBCSpaceDensityDialog;
import com.femsfe.GUIDialogBox.DefinePermanentFieldDialog;
import com.femsfe.GUIDialogBox.DefineProblemTypeDialog;
import com.femsfe.GUIDialogBox.LinePlotResultDialog;
import com.femsfe.GUIDialogBox.ProgressMeshDialog;
import com.femsfe.Geometrias.Arc2D;
import com.femsfe.Geometrias.BezierCurve2D;
import com.femsfe.Geometrias.Border2D;
import com.femsfe.Geometrias.Circle2D;
import com.femsfe.Geometrias.Face2D;
import com.femsfe.Geometrias.Geometry2D;
...
import com.femsfe.Triangulacoes.ConstrainedDelaunayTriangulation;
...
public class MainController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		menuItemOpenProject.setGraphic(new ImageView(new Image(getClass().getResourceAsStream("icon/open_file_16x16.png"))));
		menuItemNewProject.setGraphic(new ImageView(new Image(getClass().getResourceAsStream("icon/new_icon_16x16.png"))));
		menuItemSaveProject.setGraphic(new ImageView(new Image(getClass().getResourceAsStream("icon/save_icon_16x16.png"))));
		menuItemSaveProjectAs.setGraphic(new ImageView(new Image(getClass().getResourceAsStream("icon/save_as_icon_16x16.png"))));
		menuItemExit.setGraphic(new ImageView(new Image(getClass().getResourceAsStream("icon/off_16x16.png"))));
	...
	public void onNewMaterial(){
		...
		stage.getIcons().add(new Image(getClass().getResourceAsStream("/resources/icon/new_material_32x32.png")));
	...
	public void onEditMaterial(){
		...
		stage.getIcons().add(new Image(getClass().getResourceAsStream("icon/edit_material_32x32.png")));
	...
	public void onExecute(){
		...
		try {
			...
			Alert alert = new Alert(AlertType.WARNING);
			Stage stage = (Stage) alert.getDialogPane().getScene().getWindow();
			stage.getIcons().add(new Image(getClass().getResourceAsStream("icon/memory_32x32.png")));
	...
	public void onAssignWavelength(){
		...
		stage.getIcons().add(new Image(getClass().getResourceAsStream("icon/new_material_32x32.png")));
	...
	public void onLaplacianSmooth(){
		...
		stage.getIcons().add(new Image(this.getClass().getResource("icon/laplacian_smooth_24x24.png").toString()));
	...
	public void onCheckIsolines(){
		...
		if(TGLBTN_ISOLINE.isSelected()){
			...
			numberContourDialog.setGraphic(new ImageView(this.getClass().getResource("icon/isolines_32x32.png").toString()));
			
			
			Stage stage = (Stage) numberContourDialog.getDialogPane().getScene().getWindow();
			stage.getIcons().add(new Image(this.getClass().getResource("icon/isolines_32x32.png").toString()));
			...
			if(result.isPresent()){
				...
				mesh.setIsolinesVisible(false);TGLBTN_ISOLINE.setSelected(false);
				for (Face2D face2d : Project.getFaceList()) {
	...
	public void onDefineUnit(){
		...
		choiceUnit.setGraphic(new ImageView(new Image(getClass().getResourceAsStream("icon/unit_16x16.png"))));
				...
				while ( (line = br.readLine()) != null) {
					...
					switch (geoType) {
						...
						case FACE:
							...
							if(face.getTypeFace() == Face2D.EXTERNAL){ BTN_CEFACE.setDisable(true);}
	...
	public void onPlotHistogram(){
		...
		meshInfo.setVgap(5);meshInfo.setHgap(5);
		...
		histInfo.setVgap(5);histInfo.setHgap(5);
	...
	private void createFaceByEscape(){
		if(SelectGeometry.started() && SelectGeometry.isEmpty()){
			...
			if(!lastButtonID.name().trim().contains("FACE")){ return;}
